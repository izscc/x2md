...
- 下载对应平台包：
  ...
  - Windows Beta: [`X2MD_Windows_Beta.zip`](https://github.com/izscc/x2md/releases/latest/download/X2MD_Windows_Beta.zip)

当前 **Mac 为 stable 支持平台**（v4 stable 仍须通过 Developer ID/notarization 人工门禁）。
Windows 为自带 Bun 运行时的 TypeScript beta，不需要预装 Node/Python，也不包含 tray、设置窗口或 autostart。
能力和恢复 stable 的门禁见
...
2. 首次启动会打开 Setup Doctor；依次检查运行环境、保存目录、扩展和本地样例保存。
3. 设置 Markdown 保存目录；视频、图片和重复策略可在桌面设置中统一调整。
4. 本地服务固定监听 `127.0.0.1:9527`，不需要也不能自定义端口。
...
5. 在扩展连接页输入桌面设置显示的 6 位配对码；配对码单次有效，凭据失效时重新配对。
...
优先使用桌面设置修改配置。配置文件位于 Mac 用户应用目录
`~/Library/Application Support/X2MD/config.json`，当前 schema 会自动迁移旧字段。常用字段：
...
- `duplicate_policy`: 重复内容处理，支持 `skip`（默认）、`update`、`always_new`
- `download_images` / `image_attachment_path` / `image_embed_style`: 图片本地化策略
...
- 若服务在线但提示认证失效，在桌面设置读取新配对码并重新配对；不要修改端口。
...
### 4) 批量书签或博主任务关闭页面后在哪里看

- 打开桌面设置的“任务中心”。任务状态保存在 App 中，页面关闭或 MV3 worker 休眠不会清空进度。
- 可暂停、继续、取消或只重试失败项；rate limit 会自动暂停并显示原因。
- 已保存和已跳过项不会重复执行。完成后可打开结果目录，失败摘要可复制。

### 5) 重复保存如何处理

- 默认 `skip`：跳过并返回已有文件，可直接在 Finder/Obsidian 打开或复制路径。
- `update`：更新已索引文件；`always_new`：始终生成新文件。三种策略在桌面设置统一配置。

## 本地开发与打包

Mac 默认使用 Electrobun + Bun/TypeScript：
...
Windows beta 同样使用 `app/core` TypeScript 核心，不回退 Python。
...
```text
.
├── app/                    # Electrobun/Bun TypeScript 桌面端
...
- 新增 Bookmarks/Profile 批量导出进度控制、Quote/Retweet 语义、升级提示和 release artifact 校验（旧 Windows Lite 已在 v4 移除）。
