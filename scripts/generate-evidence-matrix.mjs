...
const rows = [
  ...
  ["tag、App、扩展、artifact、README、live ping 版本一致。", "待验证", "T41 最终 artifact smoke"],
  ...
  ["多目录附件引用全部有效。", "待验证", "尚缺多目标目录附件 golden；T45 前补齐"],
  ...
  ["PR/main CI 全量运行且无 skipped gate。", "待验证", "T41 release workflow gate"],
  ["Mac stable 通过签名、公证和真实 artifact smoke。", "人工门禁 H02", "需要 Developer ID、notary accepted、stapler validate 证据"],
  ["Windows 只有在 TS artifact smoke 通过后标记 stable。", "待验证", "T43 windows-latest workflow"],
  ["GitHub Release 包含 SHA、SBOM 和 provenance。", "待验证", "T44 release provenance"],
];
...
const target = resolve("docs/acceptance/v4-evidence-matrix.md");
mkdirSync(dirname(target), { recursive: true });
writeFileSync(target, output, "utf8");
console.log(`generated ${target} (${rows.length} criteria)`);
